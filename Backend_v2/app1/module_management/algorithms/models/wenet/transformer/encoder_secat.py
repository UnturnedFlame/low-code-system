...
from wenet.transformer.semodule import SEModule
...
class BaseEncoder(torch.nn.Module):
    def __init__(
        ...
        self.se_block = SEModule(output_size * num_blocks)
    ...
    def forward(
        ...
        for layer in self.encoders:
            ...
            out.append(xs)
        #连接所有层的输出
        xs = torch.cat(out, dim=-1)#(B, T/subsampling rate, D)
        xs = self.se_block(xs, xs_lens//2)
